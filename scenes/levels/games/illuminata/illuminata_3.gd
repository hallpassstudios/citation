extends Node2D

onready var camera
onready var spawn = $"player spawn"
var player = preload("res://scenes/player/side_scroll_player.tscn")

var area_1_played = false
var area_2_played = false
var area_3_played = false
var area_4_played = false

func _ready():
	#spawn player
	player = player.instance()
	add_child(player)
	player.position = spawn.position
	camera = player.get_child(0)
	
	# fade in the level
	global_ui.fade_in()
	
	yield(get_tree().create_timer(2.0), "timeout")
	globals.illuminata_completed = true
	camera.zoom_in(player.position - camera.get_camera_position())

func _on_Area2D_body_entered(body):
	if body.name == "player"  && !area_1_played:
		
		print(area_1_played)
		area_1_played = true
		dialogue_controller.play_dialogue('illuminata 3 movement')

func _on_Area2D2_body_entered(body):
	if body.name == "player"  && !area_2_played:
		dialogue_controller.play_dialogue('illuminata 3 movement 2')
		area_2_played = true

func _on_Area2D3_body_entered(body):
	if body.name == "player"  && !area_3_played:
		camera.zoom_out(Vector2(0,0))
		dialogue_controller.play_dialogue('illuminata 3 movement 3')
		area_3_played = true

func _on_Area2D4_body_entered(body):
	if body.name == "player"  && !area_4_played:
		dialogue_controller.play_dialogue('illuminata 3 movement 4')
		area_4_played = true

func _on_death_body_entered(body):
	if body.name == "player": #or if it's an answer
		print("player fell")
		global_ui.fade_out()
		yield(get_tree().create_timer(1.0), "timeout")
		get_tree().reload_current_scene()
		global_ui.fade_in()
	if body.is_in_group("correct"):
		dialogue_controller.play_dialogue('correct fell')
		get_tree().reload_current_scene()

# mobile controls
func _on_left_button_pressed():
	player.left_pressed()

func _on_right_button_pressed():
	player.right_pressed()

func _on_jump_button_pressed():
	player.jump()

func _on_left_button_released():
	player.left_released()

func _on_right_button_released():
	player.right_released()
